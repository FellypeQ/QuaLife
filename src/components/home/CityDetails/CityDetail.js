...
import IndexCity from "../IndexCity/IndexCity";
...
function CityDetail(props) {
  ...
  // console.log("COMPARE :", compareOne);
  console.log("IMAGE", imageCompare);
  console.log("compareOne", compareOne);
  async function getCompare(cityCompare) {
    try {
      const response = await axios.get(
        `https://api.teleport.org/api/urban_areas/slug:${cityCompare}/details/`
      );

      const responseImage = await axios.get(
        `https://api.teleport.org/api/urban_areas/slug:${cityCompare}/images/`
      );
      response.imagem = responseImage.data.photos[0].image.mobile;
      console.log("response", response);
      setCompareOne(compareOne.concat(response));
      // setimageCompare(responseImage.data.photos[0].image.mobile);
      setimageCompare(
        imageCompare.concat(responseImage.data.photos[0].image.mobile)
      );
      // console.log("new compareone :", compareOne);
    } catch (error) {
      console.log(error);
    }
  }

  const [getWordforCompare, setWordforCompare] = useState();
  function handleChange(event) {
    // console.log("HANDLECHANGE :", event.currentTarget.value);
    setWordforCompare(event.currentTarget.value);
  }
  ...
  return (
    <div>
      <div className="compare">
        ...
        >
          {acabou.map((elem) => {
            return <option value={elem.href.slice(46, -1)}>{elem.name}</option>;
          })}
        </select>
        <button
      ...
      <div className="flex">
        <IndexCity date={getDetails} />
        <ComponentCity data={getDetails} img={getImage} />
        {compareOne.map((elem) => {
          return (
            <ComponentCity data={elem.data.categories} img={elem.imagem} />
          );
        })}
      </div>
    </div>
